...
In unserer Gruppe hat es problemlos geklappt. Wir konnten alle Schritte ausführen,sodass am Schluss nur noch 11 Treffer angezeigt werden.

### Grafik
Im Laufe des Kurses hatten wir Koha, Archivesspace und DSpace installiert. Bei all diesen Programmen hatten wir jeweils ein wenig Content erstellt. Bei Koha hatten wir ein Medium und einen Nutzer erstellt, bei Archivesspace eine Entität und Ressource??» und bei Dspace ein repository mit einem Dokumentenupload. Über die Schnittstelle OAI-PMH und dem Programm Vufind harvest haben wir die erstellten Daten in Koha und Archivesspace geharvestet oder anders gesagt die Daten "gesammelt" und heruntergeladen. Bei DSpace haben wir die Daten direkt aus der OAI-Schnittstelle der Website geladen. Das Programm Vufindharvest haben wir über das Terminal installiert. Nun waren diese geharvesten Daten aber noch nicht in verschiedenen Metadatenstandards, in EAD, dublin core , . Um diese zu vereinheitlichen haben wir das Programm marcEdit (XSLT) verwendet. alle sollen im xml format sein Heute haben wir all diese transformierten Daten mit einem import Skript von Vufind in den Suchindex(Solr) geladen.
